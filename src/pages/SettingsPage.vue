...
<template><section class="page-stack"><header class="page-header"><div><span>Góc riêng của mình</span><h1>Những điều nhỏ để giữ thương</h1></div></header><section class="soft-card"><strong>Cách mình giữ yêu thương</strong><ul class="guide-list"><li v-for="item in guideItems" :key="item.title"><component :is="item.icon" :size="18" /><div><strong>{{ item.title }}</strong><p>{{ item.text }}</p></div></li></ul></section><section class="soft-card"><strong>Google Photos</strong><p>Ảnh kỷ niệm chính được quản lý trong shared album, app chỉ lưu link gắn với kỷ niệm/nhật ký.</p><button class="primary-btn" @click="openAlbum"><ExternalLink :size="18" /> Album của hai đứa mình</button><button class="ghost-btn" @click="openAlbum">Thêm ảnh thương nhớ</button></section><section class="soft-card"><strong>Giao diện</strong><div class="segmented"><button :class="{ selected: mode === 'light' }" @click="setTheme('light')"><SunMedium :size="16" /> Sáng</button><button :class="{ selected: mode === 'dark' }" @click="setTheme('dark')"><Moon :size="16" /> Tối</button><button :class="{ selected: mode === 'system' }" @click="setTheme('system')">System</button></div></section><section class="soft-card"><strong>Vị trí realtime</strong><p>Vị trí realtime hoạt động tốt nhất khi ứng dụng đang mở. PWA trên iOS/Android không đảm bảo GPS nên khi trình duyệt bị đóng hoặc app bị suspend.</p></section><button class="danger-btn" @click="logout"><LogOut :size="18" /> Đăng xuất</button></section></template>
