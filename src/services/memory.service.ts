...
export const memoryService = {
  async list(coupleId: string, search = '', year = '') {
    ...
    const { data, error } = await query.returns<Memory[]>()
    if (error) throw error
    return data
  },
  async get(id: string) {
    const { data, error } = await supabase.from('memories').select('*').eq('id', id).single<Memory>()
    if (error) throw error
    return data
  },
  async create(input: MemoryInput) {
    const { data, error } = await supabase.from('memories').insert(input).select().single<Memory>()
    if (error) throw error
    return data
  },
  async update(id: string, input: Partial<MemoryInput>) {
    const { data, error } = await supabase.from('memories').update(input).eq('id', id).select().single<Memory>()
    if (error) throw error
    return data
  },
